...
void work(void* pdata) {
	...
	//struct object_s**objs2= (struct objects_s*)(kmalloc(2*sizeof(struct objects_s) * data.iterations));
	/*struct objects_s *objs2[50]; 
	for (int i = 0; i < 50; i++) {
		objs2[i]= (struct objects_s*)(kmalloc(sizeof(struct objects_s) * data.iterations));
	}
	for (int i = 0; i < 50; i++) {
		kfree(objs2[i]);
	}*/
